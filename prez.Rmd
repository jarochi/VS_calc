...
## Congo red assay and calcofluor assay
...
cr_df <- mutate(cr, 
  ...
  select(-pathotype) %>% 
  melt(id.vars = "strain") %>% 
  mutate(value = factor(value, labels = c("bdar", "quality", "rdar", "smooth")),
         value = relevel(value, "quality"))

ggplot(cr_df, aes(x = strain, y = variable, fill = value)) +
  geom_tile(color = "black") +
  scale_x_discrete("Strain") +
  scale_y_discrete("Date") +
  scale_fill_discrete("Phenotype") +
  theme_bw()

```

Why there are no CR measurements for 5275 and 5276?
