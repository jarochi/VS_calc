...
=======
library(grid)
>>>>>>> 8bce5069b5b56e29c1bd6d5135e7e8686697718f
...
To account for normalization and baselining, we are using Spearman's correlation coefficient which relies on ranks instead of raw values.s
...
## CV: correlation of median results
...
## Correlation of the data {.columns-2}
```{r}
...
group_by(VS_aggr, medium) %>% 
   summarise(rho = cor(R2017, R2018, method = "pearson")) %>% 
    knitr::kable()
```
