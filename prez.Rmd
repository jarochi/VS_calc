...
## Crystal Violet - LB ~ strain comparision
...
```{r}
data.table(group_by(CV_all, strain, date, replicate) %>% 
  summarise(value = median(value)) %>% 
  summarise(value = median(value)) %>% 
  dcast(strain ~ date))
```
...
## KOZA | species ~ medium comparision
```{r}
...
## ANIA | species ~ medium comparision
```{r}
...
## Congo red assay and Calcofluor assay comparision
```{r}
